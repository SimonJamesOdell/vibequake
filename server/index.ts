...
import type { ArenaMap, ArenaMapSummary, ArenaVisualTheme, HighScoreEntry, HighScoreSubmission, MapImportRequest } from '../src/shared/contracts.js'
import { legacyTilesToBrushMap, type LegacyTileMap } from '../src/shared/legacy-map.js'
...
const assertApprovedOpenLicense = (value: string) => {
...
const APPROVED_VISUAL_THEMES = new Set<ArenaVisualTheme>(['neon-tech', 'gothic-dungeon', 'industrial-rust'])

const normalizeVisualTheme = (value: unknown): ArenaVisualTheme | undefined => {
  if (typeof value !== 'string') {
    return undefined
  }
  if (!APPROVED_VISUAL_THEMES.has(value as ArenaVisualTheme)) {
    throw new Error(`Map visualTheme must be one of: ${Array.from(APPROVED_VISUAL_THEMES).join(', ')}`)
  }
  return value as ArenaVisualTheme
}
...
const validateBrushMap = (value: unknown): ArenaMap => {
  if (!value || typeof value !== 'object') {
    throw new Error('Map payload must be an object.')
  }

  const candidate = value as Partial<ArenaMap>
...
const normalizeMapPayload = (value: unknown, overrides?: Partial<Pick<ArenaMap, 'id' | 'name' | 'description' | 'updatedAt' | 'license' | 'sourceUrl' | 'attribution'>>): ArenaMap => {
  if (!value || typeof value !== 'object') {
    throw new Error('Map payload must be an object.')
  }
  ...
  if (Array.isArray(candidate.tiles)) {
    const legacy = candidate as unknown as LegacyTileMap
...
const validateHighScoreSubmission = (value: unknown): HighScoreSubmission => {
  if (!value || typeof value !== 'object') {
    throw new Error('High score submission must be an object.')
  }
  const candidate = value as Partial<HighScoreSubmission>
...
const validateMapImportRequest = (value: unknown): MapImportRequest => {
  if (!value || typeof value !== 'object') {
    throw new Error('Map import request must be an object.')
  }

  const candidate = value as Partial<MapImportRequest>
  if (!candidate.url) {
...
const listMapSummaries = async (): Promise<ArenaMapSummary[]> => {
...
const readHighScores = async (): Promise<HighScoreEntry[]> => {
  const contents = await fs.readFile(highScoresPath, 'utf8')
  const parsed = JSON.parse(contents) as HighScoreEntry[]
  return parsed
...
const writeHighScores = async (scores: HighScoreEntry[]) => {
