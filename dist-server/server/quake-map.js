...
const convertPoint = (source, centerX, centerY, minZ, scale) => ({
...
const parseEntityProperty = (line) => {
...
const buildBrushFaces = (vertices, planes) => {
    const faces = [];
    for (const plane of planes) {
        const facePoints = dedupePoints(vertices.filter((point) => Math.abs(dot(plane.normal, point) - plane.distance) <= 0.04));
        if (facePoints.length < 3) {
            continue;
        }
        const center = scaleVec(facePoints.reduce((sum, point) => add(sum, point), toVec3(0, 0, 0)), 1 / facePoints.length);
...
const parseQuakeEntities = (source) => {
    const entities = [];
...
const fallbackEnemySpawns = (boundsMin, boundsMax, spawnY) => {
...
export const parseQuakeMapSource = (source, details) => {
    ...
    const renderEntities = entities.filter((entity) => entity.brushes.length > 0 && isRenderableBrushEntity(entity.properties.classname));
    ...
    const brushes = sourceBrushes.map((brush) => {
